...
<xsl:apply-templates select="//item"/>
  ...
  <xsl:template match="item">
    <xsl:if test="./a[7][contains(text(),'OBJ')]">
      <!--définir des variables: $obj pour le POS du type OBJ extrait-->
      ...
      <xsl:variable name="obj" select="./c/text()"/>
      ...
      <xsl:variable name="ecart" select="$obj_num - $gov_num"/>
        <xsl:choose>
      <!--$ecart > 0 : le gouverneur est avant son objectif, on cherche le POS précédent pour extraire le gouverneur précédent avec l'index soustrait $ecart-->
            <xsl:when test="$ecart > 0">
                <xsl:value-of select="preceding-sibling::*[$ecart]/c/text()"/><xsl:text> </xsl:text><xsl:text> --- </xsl:text>
                <xsl:value-of select="$obj"/><xsl:text>
</xsl:text>
            </xsl:when>
      <!--Sinon : le gouverneur est après son objectif, on cherche le POS suivant pour extraire le gouverneur suivant avec l'index soustrait $ecart-->
                    <xsl:otherwise>
                      <xsl:value-of select="$obj"/><xsl:text> </xsl:text><xsl:text> --- </xsl:text>
                      <xsl:value-of select="following-sibling::*[-$ecart]/c/text()"/><xsl:text>
</xsl:text>
                    </xsl:otherwise>
